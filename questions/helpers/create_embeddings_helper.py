...
def merge_embeddings_and_store(file_1,file_2):
    embeddings = HuggingFaceEmbeddings()
    vectorstore_1 = FAISS.load_local(file_1, embeddings=embeddings,allow_dangerous_deserialization=True)
    vectorstore_2 = FAISS.load_local(file_2, embeddings=embeddings,allow_dangerous_deserialization=True)
    vectorstore_1.merge_from(vectorstore_2)
    vectorstore_1.save_local("questions/data/faiss_files/temp_merged_faiss_index.faiss")