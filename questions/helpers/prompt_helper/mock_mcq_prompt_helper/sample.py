...
**Instructions:**
    ...
    Pickup all multiple-choice questions (MCQs)  from the following question content:

    Question Content: {content}

    and then create unique new  UPSC style multiple choice questions by doing the following the for each picked up question:
    1.  using the description about each options (if the options are some named entities) as subject of the recreated question 
    2.  using each incorrect option (if the options are named entities) as subject of the recreated question

    while looking for it's analysis, solution explanation and description of options in the following solution content:


    Solution Content: {solution}

    with 4 options (only 1 correct) each asking all the data and facts and named entities present in the question content   
...
4. **Question Quality:**
    ...
    * Never reproduce the question as it is from the content, always transform it into a totally new question using the options as the subject
...
* Avoid mentioning any information related to the prompt, input text, input document, LLM, LangChain, or vector embeddings 
...
* Do not produce the questions verbatim but rephrase them but do not rephrase the options
